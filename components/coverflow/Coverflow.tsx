import { FC, LegacyRef, useEffect, useLayoutEffect, useMemo, useRef, useState } from 'react'
...
const isPromiseResolution = <T,>(promise: PromiseResolution<T> | PromiseRejection): promise is PromiseResolution<T> => {
...
const Coverflow: FC<CoverflowProps> = (props) => {
  const {
    ...
    slidesPerSide = 0,
    rotation = ROTATION,
    opacityInterval = OPACITY_ORDER,
    scaleInterval = SCALE_ORDER,
  } = props
  /**
   * Sliders per side
   */
  ...
  const coverflowRef = useRef<HTMLInputElement | null>(null)
  ...
  const coverflowHeight = props.height ?? 500
  const coverflowWidth = props.width ?? 500
   ...
   ***********************/
  useEffect(() => {
    const fetchImagesFromList = async (originalImageList: Images[]) => {
      const fetchedImages = await fetchImages(originalImageList)
      const succeedImages = fetchedImages
        .filter(({ status }) => status === 'fulfilled')
        .map((item) => (isPromiseResolution(item) ? item.value : undefined))
        .filter(Boolean) as FetchedItem[] // Add type assertion here
      setImageList(succeedImages)
      setCurrentIndex(Math.floor(succeedImages.length / 2))
    }

    fetchImagesFromList(images)
  }, [images])
  ...
  useLayoutEffect(() => {
    ...
    imageList.forEach((image, index) => {
      ...
      if (distanceFromMiddle < 0) {
        // we only want to move 40% so they overlap
        edge += scaledWidth * 0.6
      } else {
        // RIGHT HAND SIDE
        ...
        if (nextImage) {
          ...
          edge += scaledWidth - nextImageScaledWidth + nextImageScaledWidth * 0.6
        } else {
          edge += scaledWidth
        }
      }
  ...
  return (
    <div className="main">
      <div className={`${className ?? ''} coverflow-wrapper`} ref={ref as LegacyRef<HTMLDivElement>}>
        <div
          ref={coverflowRef}
          className="coverflow"
          style={{
